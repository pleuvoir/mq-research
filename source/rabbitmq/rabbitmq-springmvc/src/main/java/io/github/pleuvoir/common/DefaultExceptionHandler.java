...
public class DefaultExceptionHandler extends SimpleMappingExceptionResolver {
	...
	protected ModelAndView doResolveException(HttpServletRequest request, HttpServletResponse response, Object handler,
		...
		if(isReturnJson) {
			...
			if(CollectionUtils.isEmpty(errorJson) && StringUtils.isNotBlank(defaultErrorJson)) {
				errorJson = JSON.parseObject(defaultErrorJson, HashMap.class);
			}
			//公共的异常错误json有误时，最后一层补救
			if(CollectionUtils.isEmpty(errorJson)) {
				Map<String,String> attrs = new HashMap<>();
				attrs.put("code", "ERROR");
	...
	private boolean isReturnJson(Object handler) {
		if(handler instanceof HandlerMethod) {
			...
			if(responseBody==null) {
				...
				//当controller上存在RestController注解，表示返回的内容为json
				if(controllerClass.getAnnotation(RestController.class)!=null){
					return true;
				}
				responseBody = controllerClass.getAnnotation(ResponseBody.class);
			}
			if(responseBody!=null) {
				return true;
			}
