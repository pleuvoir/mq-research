...
//使用一个队列监听所有发送到此交换机的且是A机器订单信息的消息
@RabbitListener(
...
@Service
public class AOrderConsumer {

	private static Logger logger = LoggerFactory.getLogger(AOrderConsumer.class);
	
	@RabbitHandler
	public void handler(@Payload String data, @Headers Map<String,Object> headers, Channel channel) throws IOException {
		logger.info("AOrderConsumer 已接收到消息，payload：{}", data);
	}
